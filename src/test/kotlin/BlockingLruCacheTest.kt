import lru.bench.BlockingLruCache
...
class BlockingLruCacheTest {
    ...
    @Test
    fun testStatistics() {
        val cache = BlockingLruCache<String, String>(3)
    ...
    fun testConstructorDoesNotAllowZeroCacheSize() {
        try {
            BlockingLruCache<String, String>(0)
            TestCase.fail()
    ...
    fun testToString() {
        val cache = BlockingLruCache<String, String>(3)
        TestCase.assertEquals("BlockingLruCache[maxSize=3,hits=0,misses=0,hitRate=0%]", cache.toString())
        ...
        TestCase.assertEquals("BlockingLruCache[maxSize=3,hits=3,misses=2,hitRate=60%]", cache.toString())
    }

    @Test
    fun testEvictionWithSingletonCache() {
        val cache = BlockingLruCache<String, String>(1)
        cache.put("a", "A")
        cache.put("b", "B")
    ...
    fun testCustomSizesImpactsSize() {
        val cache = object : BlockingLruCache<String, String>(10) {
    ...
    fun testEvictionWithCustomSizes() {
        val cache = object : BlockingLruCache<String, String>(4) {
            override fun sizeOf(key: String, value: String): Int {
                return value.length
            }
        }
    ...
    fun testEvictionThrowsWhenSizesAreInconsistent() {
        val cache = object : BlockingLruCache<String, IntArray>(4) {
    ...
    fun testRemoveAbsentElement() {
        val cache = BlockingLruCache<String, String>(10)
        cache.put("a", "A")
        cache.put("b", "B")
    ...
    fun testEntryRemovedIsCalledWithoutSynchronization() {
        val cache = object : BlockingLruCache<String, String>(3) {
    ...
    @Test
    fun testCreateIsCalledWithoutSynchronization() {
        val cache = object : BlockingLruCache<String, String>(3) {
    ...
    fun testCreateWithConcurrentCreate() {
        val log = ArrayList<String>()
        val cache = object : BlockingLruCache<String, Int>(3) {
            var callCount = 0
    ...
    private fun newCreatingCache(): BlockingLruCache<String, String> {
        return object : BlockingLruCache<String, String>(3) {
            override fun create(key: String): String? {
    ...
    private fun newRemovalLogCache(log: MutableList<String>): BlockingLruCache<String, String> {
        return object : BlockingLruCache<String, String>(3) {
            override fun entryRemoved(
                evicted: Boolean, key: String, oldValue: String, newValue: String?
            ) {
    ...
    private fun assertHit(cache: BlockingLruCache<String, String>, key: String, value: String) {
        TestCase.assertEquals(value, cache.get(key))
        expectedHitCount++
        assertStatistics(cache)
    }

    private fun assertMiss(cache: BlockingLruCache<String, String>, key: String) {
        TestCase.assertEquals(null, cache.get(key))
        expectedMissCount++
        assertStatistics(cache)
    }

    private fun assertCreated(cache: BlockingLruCache<String, String>, key: String, value: String) {
        TestCase.assertEquals(value, cache.get(key))
        expectedMissCount++
        expectedCreateCount++
        assertStatistics(cache)
    ...
    private fun assertStatistics(cache: BlockingLruCache<*, *>) {
    ...
    private fun <T> assertSnapshot(cache: BlockingLruCache<T, T>, vararg keysAndValues: T) {
